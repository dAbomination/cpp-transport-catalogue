...
namespace Catalogue {	
	...
	public:
		...
		// Возвращает все имена всех существующих маршрутов/остановок
		const std::set<std::string_view>& GetBuses() const;
		const std::unordered_map<std::string_view, const domain::Stop*>& GetAllStops() const;

		// Возвращает значение реального расстояния от stop1 до stop2, если такого значения нет возвращает расстояние от stop2 до stop 1
